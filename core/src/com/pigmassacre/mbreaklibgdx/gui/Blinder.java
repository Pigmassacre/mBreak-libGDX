...
public class Blinder extends Widget {
	...
	public Blinder(TextureRegion image, Stage stage, TweenManager tweenManager, boolean left, boolean right, boolean up, boolean down) {
		...
		List<Side> choices = new ArrayList<Side>();
		if (left)
			choices.add(Side.LEFT);
		if (right)
			choices.add(Side.RIGHT);
		if (up)
			choices.add(Side.UP);
		if (down)
			choices.add(Side.DOWN);
		
		Side choice = choices.get(MathUtils.random(choices.size() - 1));
