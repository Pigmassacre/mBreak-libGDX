...
public class DebugInput extends InputAdapter {
	...
	public boolean touchDown(int screenX, int screenY, int pointer, int button) {
		...
		if (stage != null && (pointer == 0 || button == Buttons.LEFT) && rectangle.contains(coords.x, coords.y)) {
			new Ball(coords.x, coords.y, (float) (random.nextFloat() * 2 * Math.PI), (Player) Groups.playerGroup.getChildren().first(), new Color(random.nextFloat(), random.nextFloat(), random.nextFloat(), 1.0f));
		}
		return false;
	}
